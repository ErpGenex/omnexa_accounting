...
"""Idempotent Frappe Workflow setup for omnexa_accounting sales chain DocTypes."""

import frappe

# Shared Workflow State master names (one set for all chain DocTypes).
STATE_DRAFT = "Omnexa Sales Chain Draft"
STATE_SUBMITTED = "Omnexa Sales Chain Submitted"
STATE_CANCELLED = "Omnexa Sales Chain Cancelled"
...
def _build_workflow_doc(doctype: str, workflow_name: str, locked_role: str):
	...
	# allow_edit = role that may edit in that state (not who can transition).
	# Draft: broad; submitted/cancelled: narrow so submitted docs stay immutable for normal users.
...
def ensure_sales_chain_workflows() -> None:
	"""Create Workflow State / Action masters and one active workflow per sales chain DocType."""
