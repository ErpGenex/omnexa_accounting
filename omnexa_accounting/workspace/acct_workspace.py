...
"""Full Accounting workspace — global leader catalog."""
...
def _build_link_rows() -> list[dict]:
	return build_link_rows_for_app("omnexa_accounting", WORKSPACE_SECTIONS)
...
def _build_content(link_rows: list[dict], ws) -> str:
	...
	content.append(
		{
			"id": "acct-title",
			"type": "header",
			"data": {"text": '<span class="h4"><b>Accounting</b></span>', "col": 12},
		}
	)
	section_idx = 0
	link_idx = 0
	for row in link_rows:
		if row.get("type") == "Card Break":
			if section_idx:
				content.append({"id": f"acct-sp-{section_idx}", "type": "spacer", "data": {"col": 12}})
			content.append(
				{
					"id": f"acct-sec-{section_idx}",
					"type": "header",
	...
	if ws.number_cards:
		content.append({"id": "acct-kpi-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": "acct-kpi-h",
				"type": "header",
	...
	if ws.charts:
		content.append({"id": "acct-ch-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": "acct-ch-h",
				"type": "header",
...
def sync_acct_workspace_menu(*, save: bool = True, rebuild: bool = True) -> dict:
